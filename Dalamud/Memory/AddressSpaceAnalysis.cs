...
internal static unsafe class AddressSpaceAnalysis
{
    ...
    public static string BuildReport(nint address, int freeBlockCount = 10)
    {
        ...
        sb.AppendLine($"地址空间概况：{Util.DescribeAddress(address)}，共 {regions.Count} 个区域。");

        if (FindRegion(regions, anchor) is { } target)
        {
            sb.AppendLine(
                $"目标区域：0x{target.Start:X}-0x{target.End:X}（{Util.FormatBytes(target.Size)}）" +
                $"{target.StateName} 保护=0x{target.Protect:X}");
        }
        else
        {
            sb.AppendLine("目标区域：未映射");
        }

        if (window.WindowSize == 0)
        {
            sb.AppendLine("无法为此地址计算 rel32 窗口。");
            return sb.ToString();
        }
        ...
        sb.AppendLine(
            $"rel32 窗口：0x{window.WindowStart:X}-0x{window.WindowEnd:X}（{Util.FormatBytes(window.WindowSize)}），" +
            $"已耗尽 {exhaustion * 100:F2}%");
        sb.AppendLine(
            $"  可分配：      {Util.FormatBytes(allocatable)}，共 {window.AllocatableGranules} 个 64 KB 粒度块");
        sb.AppendLine($"  最大连续空间： {Util.FormatBytes(window.LargestFreeRun)}");
        sb.AppendLine($"  空闲（原始）：  {Util.FormatBytes(window.Free)}");
        sb.AppendLine($"  零散空闲空间：  {Util.FormatBytes(stranded)}");
        sb.AppendLine($"  已保留：       {Util.FormatBytes(window.Reserved)}");
        sb.AppendLine($"  已提交：       {Util.FormatBytes(window.Committed)}");

        if (window.AllocatableGranules == 0)
        {
            sb.AppendLine(
                "没有空闲的 64 KB 槽位，无法设置 Hook");
        }

        var blocks = GetLargestFreeBlocks(regions, window, freeBlockCount);
        if (blocks.Count > 0)
        {
            sb.AppendLine($"窗口中最大的 {blocks.Count} 个空闲块：");
