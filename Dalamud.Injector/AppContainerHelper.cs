    ...
    internal static class AppContainerHelper
    {
        ...
        public static void GrantAccess(string path, IntPtr sid, uint accessMask)
        {
            var err = PInvoke.GetNamedSecurityInfoW(
                ...
                out var securityDescriptor);
            if (err != 0)
                throw new Win32Exception((int)err, $"对 {path} 调用 GetNamedSecurityInfo 失败");
            ...
            try
            {
                ...
                if (err != 0)
                    throw new Win32Exception((int)err, $"对 {path} 调用 SetEntriesInAcl 失败");
                ...
                err = PInvoke.SetNamedSecurityInfoW(
                    ...
                    IntPtr.Zero);
                if (err != 0)
                    throw new Win32Exception((int)err, $"对 {path} 调用 SetNamedSecurityInfo 失败");
            }
            finally
            {
        ...
        public static bool TryAddLoopbackExemption(AppContainerLaunchContext ctx)
        {
            try
            {
                ...
                if (err != 0)
                {
                    Log.Warning("调用 NetworkIsolationGetAppContainerConfig 失败： {Err}", err);
                    return false;
                }
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    {
                        Log.Verbose("回环豁免已存在： {Sid}", ctx.ContainerSidString);
                        return true;
                    }
                }
                ...
                try
                {
                    ...
                    if (err != 0)
                    {
                        Log.Warning(
                            "无法添加回环豁免（错误 {Err}）。 " +
                            "请确保以管理员身份运行，或改为以管理员身份运行一次：CheckNetIsolation.exe LoopbackExempt -a -p={Sid}",
                            err,
                            ctx.ContainerSidString);
                        return false;
                    }

                    Log.Information("已添加回环豁免： {Sid}", ctx.ContainerSidString);
                    return true;
                }
                finally
                {
