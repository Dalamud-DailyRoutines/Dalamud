...
namespace Dalamud.Injector
{
    ...
    internal sealed class SandboxConfiguration
    {
        ...
        public static SandboxConfiguration Load(string path, bool mustExist)
        {
            ...
            {
                if (mustExist)
                    throw new FileNotFoundException($"在 {path} 处未找到沙盒配置。", path);

                Log.Verbose("[SANDBOX] 在 {Path} 处未找到沙盒配置，使用默认设置", path);
            ...
            catch (Exception ex)
            {
                throw new InvalidDataException(
                    $"无法读取位于 {path} 的沙盒配置。 " +
                    "请修复该文件，或传递 --no-sandbox 以不使用沙盒启动。",
                    ex);
            }
            ...
            Log.Information("[SANDBOX] 已从 {Path} 加载沙盒配置", path);
            return loaded;
        }
